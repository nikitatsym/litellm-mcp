...
def _error_result(exc: ValueError | APIError | httpx.RequestError) -> dict[str, str]:
    if isinstance(exc, httpx.RequestError):
        ...
        return {
            "error": (
                f"LiteLLM transport failure: {method} {path}: "
...
def _safe_tool(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    The wrapper mirrors `fn`'s sync/async flavor: MCP classifies a tool with
    `inspect.iscoroutinefunction`, so making a sync tool awaitable would move
    its blocking HTTP call out of the SDK worker thread onto the event loop.
    """
...
def _compact(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    if inspect.iscoroutinefunction(fn):
        ...
        async def async_compact(*args: Any, **kwargs: Any) -> Any:
            return to_content(await cast("Awaitable[Any]", fn(*args, **kwargs)))
...
def _register_tools() -> None:
    ...
    for _importer, modname, _ispkg in pkgutil.walk_packages(
        ...
        for attr_name, raw_fn in inspect.getmembers(module, inspect.isfunction):
            ...
            if group is ROOT:
                mcp.tool(structured_output=False)(_compact(_safe_tool(raw_fn)))
            else:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="litellm-mcp",
        description="MCP server for LiteLLM. Serves MCP over stdio unless --http is given.",
    )
    parser.add_argument(
        "--http",
        action="store_true",
